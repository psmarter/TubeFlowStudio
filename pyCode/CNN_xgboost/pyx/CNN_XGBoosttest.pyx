...
class CNN1DFeatureExtractor(nn.Module):
    def __init__(self, input_dim):
        ...
        flattened_dim = 16 * (input_dim // 2)  # 确保维度正确
        self.fc1 = nn.Linear(flattened_dim, input_dim)  # 输出维度与输入一致
...
def save_models(feature_extractor, xgb_models, file_path):
    ...
    torch.save(models_dict, file_path, _use_new_zipfile_serialization=False)
