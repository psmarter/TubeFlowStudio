...
def write_mse_to_file(mse, mse_columns, output_data_path, file_name='mseoutput.txt'):
    with open(file_name, 'w') as f:
        ...
        labels, output_data = CNNtest.read_txt_file(output_data_path)
        if len(labels) == 1:
...
if __name__ == "__main__":
    ...
    #     input_header, input_data = CNNtest.read_txt_file(datainputPath)
    #     output_header, output_data = CNNtest.read_txt_file(dataoutputPath)
    #     input_size = input_data.shape[1]
    #     output_size = output_data.shape[1]
    #     print(f'Loaded input data with shape {input_data.shape[1]} and output data with shape {output_data.shape[1]}')
    #
    #     model = CNNtest.CNN1D(input_size, output_size)
    #     # model = CNNtest.CNN1D(4, 3)
    ...
    #
    #     predictions = CNNtest.predict_new_data(model, new_data_path, scaler_input, scaler_output)
    #
    ...
    #
    # else:
    X, y, mse, mse_columns, losses, y_test_real, y_pred_real = CNNtest.train_and_predict(datainputPath, dataoutputPath,
    ...
    #CNNtest.plot_predictions(y_test_real, y_pred_real, mse_columns, dataoutputPath, start_idx=0, end_idx=100)
