...
class CNN1D(nn.Module):
    def __init__(self, input_size, output_size):
        ...
        self.fc1 = nn.Linear(16 * (input_size // 2), 100)
...
def train_and_predict(input_data_path, output_data_path, model_save_path='cnn1d_model.pth'):
    ...
    model = CNN1D(X_train.shape[1], y_train.shape[1])
...
def predict_new_data(new_data_path, model_save_path='cnn1d_model.pth'):
    ...
    model = CNN1D(new_data_tensor.shape[1], 1)  # 假设输出维度为1
